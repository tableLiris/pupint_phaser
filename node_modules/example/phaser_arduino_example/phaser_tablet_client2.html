<!doctype html>
<html lang="en">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
        <title>World3</title>
        <meta name="description" content="">
        <meta name="viewport" content="width=device-width">
    </head>
    <body>
	<div style="position: relative;">
	</div>
	<div>
		<script src="js/phaser.min.js"></script>
        <script src="../libs/prototype-1.6.0.2.js"></script>
        <script src="../libs/jquery-1.7.2.js"></script>
        <script src="../libs/requestanimationframe.js"></script>
        <script src='../js/box2d/common/b2Settings.js'></script>
        <script src='../js/box2d/common/math/b2Vec2.js'></script>
        <script src='../js/box2d/common/math/b2Mat22.js'></script>
        <script src='../js/box2d/common/math/b2Math.js'></script>
        <script src='../js/box2d/collision/b2AABB.js'></script>
        <script src='../js/box2d/collision/b2Bound.js'></script>
        <script src='../js/box2d/collision/b2BoundValues.js'></script>
        <script src='../js/box2d/collision/b2Pair.js'></script>
        <script src='../js/box2d/collision/b2PairCallback.js'></script>
        <script src='../js/box2d/collision/b2BufferedPair.js'></script>
        <script src='../js/box2d/collision/b2PairManager.js'></script>
        <script src='../js/box2d/collision/b2BroadPhase.js'></script>
        <script src='../js/box2d/collision/b2Collision.js'></script>
        <script src='../js/box2d/collision/Features.js'></script>
        <script src='../js/box2d/collision/b2ContactID.js'></script>
        <script src='../js/box2d/collision/b2ContactPoint.js'></script>
        <script src='../js/box2d/collision/b2Distance.js'></script>
        <script src='../js/box2d/collision/b2Manifold.js'></script>
        <script src='../js/box2d/collision/b2OBB.js'></script>
        <script src='../js/box2d/collision/b2Proxy.js'></script>
        <script src='../js/box2d/collision/ClipVertex.js'></script>
        <script src='../js/box2d/collision/shapes/b2Shape.js'></script>
        <script src='../js/box2d/collision/shapes/b2ShapeDef.js'></script>
        <script src='../js/box2d/collision/shapes/b2BoxDef.js'></script>
        <script src='../js/box2d/collision/shapes/b2CircleDef.js'></script>
        <script src='../js/box2d/collision/shapes/b2CircleShape.js'></script>
        <script src='../js/box2d/collision/shapes/b2MassData.js'></script>
        <script src='../js/box2d/collision/shapes/b2PolyDef.js'></script>
        <script src='../js/box2d/collision/shapes/b2PolyShape.js'></script>
        <script src='../js/box2d/dynamics/b2Body.js'></script>
        <script src='../js/box2d/dynamics/b2BodyDef.js'></script>
        <script src='../js/box2d/dynamics/b2CollisionFilter.js'></script>
        <script src='../js/box2d/dynamics/b2Island.js'></script>
        <script src='../js/box2d/dynamics/b2TimeStep.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2ContactNode.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2Contact.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2ContactConstraint.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2ContactConstraintPoint.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2ContactRegister.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2ContactSolver.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2CircleContact.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2Conservative.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2NullContact.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2PolyAndCircleContact.js'></script>
        <script src='../js/box2d/dynamics/contacts/b2PolyContact.js'></script>
        <script src='../js/box2d/dynamics/b2ContactManager.js'></script>
        <script src='../js/box2d/dynamics/b2World.js'></script>
        <script src='../js/box2d/dynamics/b2WorldListener.js'></script>
        <script src='../js/box2d/dynamics/joints/b2JointNode.js'></script>
        <script src='../js/box2d/dynamics/joints/b2Joint.js'></script>
        <script src='../js/box2d/dynamics/joints/b2JointDef.js'></script>
        <script src='../js/box2d/dynamics/joints/b2DistanceJoint.js'></script>
        <script src='../js/box2d/dynamics/joints/b2DistanceJointDef.js'></script>
        <script src='../js/box2d/dynamics/joints/b2Jacobian.js'></script>
        <script src='../js/box2d/dynamics/joints/b2GearJoint.js'></script>
        <script src='../js/box2d/dynamics/joints/b2GearJointDef.js'></script>
        <script src='../js/box2d/dynamics/joints/b2MouseJoint.js'></script>
        <script src='../js/box2d/dynamics/joints/b2MouseJointDef.js'></script>
        <script src='../js/box2d/dynamics/joints/b2PrismaticJoint.js'></script>
        <script src='../js/box2d/dynamics/joints/b2PrismaticJointDef.js'></script>
        <script src='../js/box2d/dynamics/joints/b2PulleyJoint.js'></script>
        <script src='../js/box2d/dynamics/joints/b2PulleyJointDef.js'></script>
        <script src='../js/box2d/dynamics/joints/b2RevoluteJoint.js'></script>
        <script src='../js/box2d/dynamics/joints/b2RevoluteJointDef.js'></script>
        <script src='../libs/draw_world.js'></script>
        <script src="../libs/lodash.js"></script>
        <script src="../libs/socket.io.js"></script>
        <script src="../dist/Tuio.js"></script>
        <script src="../dist/hammer.js"></script>
        <script src="../dist/Pattern.js"></script>
        <script src="../dist/Shape.js"></script>
        <script src="../dist/Circle.js"></script>
        <script src="../dist/Rectangle.js"></script>
        <script src="../dist/Manager.js"></script>
	</div>
        <script>
screenW = $(window).width();
screenH = $(window).height();
var game = new Phaser.Game(screenW, screenH, Phaser.AUTO, '', { preload: preload, create: create, update: update });

var player;
var platforms;
var cursors;
var stars;
var score = 0;
var scoreText;
var positionX;
var positionY;
var posX = 0;
var posY = 0;
var msg;
var spriteExist;

function preload() {
	game.stage.backgroundColor = '#DDA0DD';
    game.load.image('ground', 'assets/platform_client3.png');
    game.load.image('star', 'assets/star.png');
    game.load.spritesheet('dude', 'assets/dude.png', 32, 48);
	game.load.image('brick', 'assets/brick.png');
}

function create() {
	//Enable the Arcade Physics system
    game.physics.startSystem(Phaser.Physics.ARCADE);

	//The platforms group contains the ground, the ledges and the brick wall
    platforms = game.add.group();
	
	//Enable physics for any object in the platforms group
    platforms.enableBody = true;
	
	//Create brick wall
	var brick = platforms.create(game.world.width-50, 0, 'brick');
	brick.body.immovable = true;
	
	//Create the ground.
	var ground = platforms.create(0, game.world.height - 64, 'ground');
    
	//Scale it to fit the width of the game 
	ground.scale.setTo(4, 4);
    
	//This stops it from falling away when we jump on it
	ground.body.immovable = true;	
	
	//Create an invisible roof
	ground = platforms.create(0, -130, 'ground');
	ground.scale.setTo(4, 4);
	ground.body.immovable = true;
	
	//Create ledges
    var ledge = platforms.create(600, 500, 'ground');
    ledge.body.immovable = true;
    ledge = platforms.create(110, 250, 'ground');
    ledge.body.immovable = true;
	ledge = platforms.create(game.world.width-450, 150, 'ground');
    ledge.body.immovable = true;

	//Create a group of stars
    stars = game.add.group();
    stars.enableBody = true;
	
	//Create 18 of them, ranged radomly
    for (var i = 0; i < 18; i++)
    {
        var starHeight = game.world.height - Math.floor(Math.random() * 600 + 100);
        var star = stars.create( i*70, starHeight, 'star');
    }
	
	//If stars are created overlaping with platforms, kill them
	game.physics.arcade.overlap(stars, platforms, deleteStar,null,this);
	
	//The score
    scoreText = game.add.text(55, 16, 'Score: 0', { fontSize: '32px', fill: '#000' });
	
	//Position of player
	positionX = game.add.text(55, 46, 'PositionX: 0', { fontSize: '32px', fill: '#000' });
	positionY = game.add.text(55, 76, 'PositionY: 0', { fontSize: '32px', fill: '#000' });
    
	//Control of keyborad
	cursors = game.input.keyboard.createCursorKeys();
}

function update() {
	if(spriteExist == true){
		//Collide the player with the platforms
		game.physics.arcade.collide(player, platforms);

		//If the player overlaps with any of the starts, call the collectStar function
		game.physics.arcade.overlap(player, stars, collectStar, null, this);
				
		//Update position of the player
		positionX.text = "PositionX: " + Math.floor(player.position.x);	
		positionY.text = "PositionY: " + Math.floor(player.position.y);
		
		//Give the player a deceleration on X axis
		player.body.drag.x  = 80;
		
		//Control by keyborad
		/*if (cursors.left.isDown)
		{
			player.body.velocity.x = -150;
			player.animations.play('left');
		}
		else if (cursors.right.isDown)
		{
			player.body.velocity.x = 150;
			player.animations.play('right');
		}
		else
		{
			player.animations.stop();
			player.frame = 4;
		}
		if (cursors.up.isDown && player.body.touching.down)
		{
			player.body.velocity.y = -350;
		}*/
		
		//Animation of the player
		if(player.body.velocity.x<-10){
			player.animations.play('left');
		}else if(player.body.velocity.x>10){
			player.animations.play('right');
		}else{
			player.animations.stop();
			player.frame = 4;
		}
		
		//When the player reaches the left border of the game world, we kill it and send the message of its position and velocity
		if (player.position.x  <= 0)
		{
			
			msg = {identifiation: 'tablet', direction: 'tablet_left', playerPositionX: player.position.x, playerPositionY: player.position.y, screenHigh: player.position.y/(screenH-112), playerBodyVelocityX: player.body.velocity.x, playerBodyVelocityY: player.body.velocity.y};
			socket.emit('addSprite',msg);
			killSprite(player);
		};
		
	}
}

// If the stars overlap with platform, then delete them
function deleteStar (star){
	star.kill();
}

// Collect star and add scores
function collectStar (player, star) {
    star.kill();
    score += 10;
    scoreText.text = 'Score: ' + score;
}

//If the player move out of the game world, we kill it
function killSprite(player){
	player.kill();
	spriteExist = false;
}


$.noConflict();
jQuery(document).ready(function($) {
	Hammer.plugins.fakeMultitouch();
	
	//connexion to the server to intercept the messages sent to the network
	socket = io.connect("192.168.1.104:1337");
	console.log("connected");
	socket.on("news", function(data) {
		pos = data.hello;
		//Send a message when first connects
		socket.emit("removeWall", {table: "tablet"});
	});	
	
	//When receive the message, make the player and add its properites
	socket.on("addSprite", function(data) {
		var obj = JSON.parse(data);
		if (obj[0].direction == 'table1_right'){
			screenW = $(window).width();
			screenH = $(window).height();
			player = game.add.sprite(1, obj[0].screenHigh*(screenH-112), 'dude');
			game.physics.arcade.enable(player);
			player.body.bounce.y = 0.2;
			player.body.gravity.y = 150;
			player.body.maxVelocity.x = 350;
			player.body.maxVelocity.y = 350;
			player.animations.add('left', [0, 1, 2, 3], 10, true);
			player.animations.add('right', [5, 6, 7, 8], 10, true);
			player.body.velocity.x = obj[0].playerBodyVelocityX;
			player.body.velocity.y = obj[0].playerBodyVelocityY;
			spriteExist = true;
		}
	});
	
	var client = new Tuio.Client({
		host: "http://192.168.1.104:5000"
	});
	
	screenW = $(window).width();
	screenH = $(window).height();
	manager = new Manager();
	def = new Pattern();
	dragLeft = false;
	dragRight = false;
	dragUp = false;
	dragDown = false;

	// Gesture recognition function
	gesture = function(event){
		if(spriteExist == true){
			if (event.type == "release") {
				if(dragLeft == true){
					player.body.velocity.x = posX;
					player.animations.play('left');
					dragLeft = false;
				}else if(dragRight == true){
					player.body.velocity.x = posX;
					player.animations.play('right');
					dragRight = false
				}
				if(dragUp == true){
					player.body.velocity.y = posY;
					dragUp = false;
				}else if(dragDown == true){
					player.body.velocity.y = posY;
					dragDown = false;
				}
				posX = 0;
				posY = 0;
			}
			else if (event.type == "drag") {
				posX = event.gesture.touches[0].pageX - event.gesture.startEvent.center.pageX;
				posY = event.gesture.touches[0].pageY - event.gesture.startEvent.center.pageY;
				if( posX!=0 ){
					if(posX>0){
						dragRight = true;
					}
					else if(posX<0){
						dragLeft = true;
					}
				}
				if(posY!= 0){
					if(posY<0){
						dragUp = true;
					}else if(posY>0){
						dragDown = true;
					}
				}/**/
			}	
		}
	};
	// Init function
	onConnect = function() {
		manager.setScreenSize(screenW, screenH);
		def.enableGesture(true);
		manager.addPattern(def);
		manager.setOnGesture(gesture);
	};
	// Refresh the screen
	onRefresh = function(time) {
	};
	// New blob event
	onAddTuioBlob = function(blb) {
		manager.addEvent(blb);
	};
	// Removed blob event
	onRemoveTuioBlob = function(blb) {
		manager.removeEvent(blb);
	};
	// Updated blob event
	onUpdateTuioBlob = function(blb) {
		manager.updateEvent(blb);
	};
	client.on("connect", onConnect);
	client.on("addTuioBlob", onAddTuioBlob);
	client.on("updateTuioBlob", onUpdateTuioBlob);
	client.on("removeTuioBlob", onRemoveTuioBlob);
	client.on("refresh", onRefresh);
	client.connect();
});
</script>
</body>
</html>
