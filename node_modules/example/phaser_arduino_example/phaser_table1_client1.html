...
function create() {
	...
	//Create the player
	player = game.add.sprite(100, game.world.height - 250, 'dude');
...
jQuery(document).ready(function($) {
	...
	//When another device connects, remove the brick wall from this game world
	socket.on("removeWall", function(data) {
		var obj = JSON.parse(data);
		if (obj[0].table == "tablet"){
			brickRight.kill();
		}
		else if (obj[0].table == "table2"){
			brickLeft.kill();
		}
	});
	...
	socket.on("addSprite", function(data) {
		...
		if ((obj[0].direction == 'tablet_left')){
			screenW = $(window).width();
			screenH = $(window).height();
			player = game.add.sprite(screenW-50, obj[0].screenHigh*(screenH-112), 'dude');
			game.physics.arcade.enable(player);
			player.body.bounce.y = 0.2;
			player.body.gravity.y = 150;
			player.body.maxVelocity.x = 350;
